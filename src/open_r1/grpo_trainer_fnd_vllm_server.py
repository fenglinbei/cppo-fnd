...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # 在 __init__ 里
        self.use_vllm_server = getattr(args, "use_vllm_server", False)

        if self.use_vllm and self.use_vllm_server:
            if self.accelerator.is_main_process:
                self.vllm_server_base_url = self.args.vllm_server_base_url.rstrip("/")
                self.vllm_server_model_name = self.args.vllm_server_model_name
                self.vllm_client = OpenAI(
                    base_url=f"{self.vllm_server_base_url}/v1",
                    api_key="EMPTY",
                )

                self._vllm_sync_initialized = False
                self._vllm_update_group = None
                self._vllm_inference_world_size = None
    ...
    def _move_model_to_vllm_server(self):
        ...
        with unwrap_model_for_generation(
            ...
            # 最小版：先按全参数训练处理
            named_params = [(n, p) for n, p in unwrapped_model.named_parameters()]

            names = [n for n, _ in named_params]
            dtype_names = [str(p.dtype).split(".")[-1] for _, p in named_params]
            shapes = [list(p.shape) for _, p in named_params]

            self._vllm_http_post("pause", timeout=60)
            ...
            update_payload = {
                "update_info": dict(
                    ...
                    shapes=shapes,
                    packed=self.args.vllm_sync_packed,
                )
            }

            update_thread = threading.Thread(
                target=self._vllm_http_post,
                args=("update_weights", update_payload, 300),
            )
            update_thread.start()

            trainer_args = NCCLTrainerSendWeightsArgs(
                group=self._vllm_update_group,
                packed=self.args.vllm_sync_packed,
            )
            ...
            update_thread.join()
            self._vllm_http_post("resume", timeout=60)
    ...
    def _generate_and_score_completions(
        ...
        if self.args.use_vllm:
            ...
            if self.state.global_step != self._last_loaded_step:
                if self.use_vllm_server:
                    self._move_model_to_vllm_server()
                else:
            ...
            if self.accelerator.is_main_process:
                ...
                if self.args.allocation:
                    ...
                    sample_n = self.num_generations * self.repeat

                    if self.use_vllm_server:
                        completion_ids = self._server_generate_completion_ids(
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                # edit
                if self.args.allocation:
                    reward_kwargs['level'] = [ x for num in reward_kwargs['level'] for x in [ num ] * self.repeat ] 
                    reward_kwargs['type'] = [ x for num in reward_kwargs['type'] for x in [ num ] * self.repeat ] 
    ...
    def _vllm_http_get(self, path: str, timeout: int = 30):
        r = requests.get(f"{self.vllm_server_base_url}/{path.lstrip('/')}", timeout=timeout)
        r.raise_for_status()
        return r.json()

    def _vllm_http_post(self, path: str, payload: dict | None = None, timeout: int = 60):
        r = requests.post(
            f"{self.vllm_server_base_url}/{path.lstrip('/')}",
            json=payload,
            timeout=timeout,
        )
        r.raise_for_status()
    ...
    def _ensure_vllm_server_sync_initialized(self):
        ...
        init_payload = {
            "init_info": dict(
                master_address=self.args.vllm_sync_master_addr,
                master_port=self.args.vllm_sync_port,
        ...
        init_thread = threading.Thread(
            target=self._vllm_http_post,
            args=("init_weight_transfer_engine", init_payload, 60),
        )
        init_thread.start()

        self._vllm_update_group = NCCLWeightTransferEngine.trainer_init(
            dict(
                master_address=self.args.vllm_sync_master_addr,
                master_port=self.args.vllm_sync_port,
                world_size=world_size,
        ...
        init_thread.join()
        self._vllm_sync_initialized = True

    def _server_generate_completion_ids(
        self,
        prompts: list[str],
        n: int,
        temperature: float,
        ...
        for prompt in prompts:
            resp = self.vllm_client.completions.create(
                model=self.vllm_server_model_name,
                prompt=prompt,
                max_tokens=self.max_completion_length,
