...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # Fact-checking specific: post-normalization class balancing for macro-F1.
        # You can set these after GRPOConfig(...) as:
        # args.factcheck_train_class_counts = [812, 1985, 1611, 2087, 1950, 1647]
        # args.factcheck_class_balance_alpha = 0.35
        self.factcheck_class_counts = getattr(args, "factcheck_train_class_counts", None)
        self.factcheck_class_balance_alpha = float(getattr(args, "factcheck_class_balance_alpha", 0.0))
        self.factcheck_class_weights = None
        if self.factcheck_class_counts is not None and self.factcheck_class_balance_alpha > 0:
            counts = torch.tensor(self.factcheck_class_counts, dtype=torch.float32)
            if len(counts) != len(ID2LABEL):
                raise ValueError(
                    f"factcheck_train_class_counts length must be {len(ID2LABEL)}, got {len(counts)}"
                )
            weights = (counts.mean() / counts).pow(self.factcheck_class_balance_alpha)
            weights = weights / weights.mean()
            self.factcheck_class_weights = weights

    def _set_signature_columns_if_needed(self):
        # Keep all dataset columns used by reward functions / evaluation, such as gold_label, claim and evidence.
        if self._signature_columns is None:
            cols = {"prompt", "completion", "gold_label", "label", "claim", "evidence", "reports", "retrieved_evidence"}
            if hasattr(self.train_dataset, "column_names"):
                cols.update(self.train_dataset.column_names)
            if isinstance(self.eval_dataset, dict):
                for ds in self.eval_dataset.values():
                    if hasattr(ds, "column_names"):
                        cols.update(ds.column_names)
            elif hasattr(self.eval_dataset, "column_names"):
                cols.update(self.eval_dataset.column_names)
            self._signature_columns = list(cols)
    ...
    def _generate_and_score_completions(
        ...
        local_gold_labels = None
        if len(inputs) > 0 and isinstance(inputs[0], dict):
            gold_key = None
            if "gold_label" in inputs[0]:
                gold_key = "gold_label"
            elif "label" in inputs[0]:
                gold_key = "label"
            if gold_key is not None:
                local_gold_labels = [int(example[gold_key]) for example in inputs]
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                if self.args.allocation:
                    ...
                    if local_gold_labels is not None:
                        local_gold_labels = [x for num in local_gold_labels for x in [num] * self.repeat]
            else:
                if self.args.allocation:
                    self.repeat = int(1 / (1-self.args.pruning))
                    prompt_ids = torch.repeat_interleave(prompt_ids, repeats=self.repeat,dim=0)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                if self.args.allocation and self.repeat > 1:
                    reward_kwargs = {
                        key: [x for item in values for x in [item] * self.repeat]
        ...
        sample_weights = None
        if local_gold_labels is not None and self.factcheck_class_weights is not None:
            gold_tensor = torch.tensor(local_gold_labels, dtype=torch.long, device=device)
            sample_weights = self.factcheck_class_weights.to(device)[gold_tensor]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if sample_weights is not None:
            advantages = advantages * sample_weights.to(advantages.device)
            self._metrics[mode]["sample_weight"].append(sample_weights.float().mean().item())
    ...
    def evaluate(
        ...
        for dataset_name, dataset_obj in target_eval_dataset.items():
            ...
            for inputs in pbar:
                ...
                if self.args.use_vllm:
                    ...
                    if self.accelerator.is_main_process:
                        ...
                        completion_ids = [out.outputs[0].token_ids for out in all_outputs]
                    else:
                        completion_ids = [None] * len(all_prompts_text)
                ...
                pred_labels = [LABEL2ID.get(extract_prediction(x)["label"], -1) for x in completions_text]
            ...
            if self.accelerator.is_main_process:
                # 某些 accelerate 版本 gather_object(list) 可能返回 list-of-lists
                if len(gathered_preds) > 0 and isinstance(gathered_preds[0], list):
                    gathered_preds = [x for sub in gathered_preds for x in sub]
                if len(gathered_golds) > 0 and isinstance(gathered_golds[0], list):
                    gathered_golds = [x for sub in gathered_golds for x in sub]
                ...
                print(
                    f"\n[{dataset_name}] "
                    f"acc={metrics[f'{prefix}_acc']:.4f} "
        ...
        metrics_list = [all_metrics if self.accelerator.is_main_process else None]
        metrics_list = broadcast_object_list(metrics_list, from_process=0)
