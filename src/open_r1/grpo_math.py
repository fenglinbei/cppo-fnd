...
from src.open_r1.rewards_math import (
    accuracy_reward,
    code_reward,
    format_reward,
...
import random 
import numpy as np
from src.open_r1.grpo_trainer_math import GRPOTrainer
...
def main(script_args, training_args, model_args):
    ...
    for eval_dataset_name in script_args.eval_dataset_names:
        if eval_dataset_name != 'aime24':
            if eval_dataset_name == 'gsm8k':
                eval_datasets[eval_dataset_name] = load_dataset(eval_datasets_path[eval_dataset_name], name='main')['test']
            else:
                eval_datasets[eval_dataset_name] = load_dataset(eval_datasets_path[eval_dataset_name], name=script_args.dataset_config)['test']
        else:
            eval_datasets[eval_dataset_name] = load_dataset(eval_datasets_path[eval_dataset_name], name=script_args.dataset_config)['train']
    ...
    if training_args.sample_num != 0:
        ...
        for k in eval_datasets.keys():
            eval_datasets[k] = eval_datasets[k].select(range(training_args.sample_num))
    
    
    dataset = dataset.map(make_conversation)

    for k in eval_datasets.keys():
        if 'solution' in eval_datasets[k].column_names:
            eval_datasets[k]= eval_datasets[k].remove_columns("answer")
        else:
            eval_datasets[k]= eval_datasets[k].rename_column("answer","solution")
            
        if 'question' in eval_datasets[k].column_names:
            eval_datasets[k]= eval_datasets[k].rename_column("question","problem")
            
        important_keys = ['problem', 'solution']
        for x in eval_datasets[k].column_names:
            if x not in important_keys:
                eval_datasets[k]= eval_datasets[k].remove_columns(x)
                
        eval_datasets[k] = eval_datasets[k].map(make_conversation)
