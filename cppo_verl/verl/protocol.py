...
from cppo_verl.verl.utils.py_functional import union_two_dict
...
class DataProto:
    ...
    def copy(self, batch_keys=None, non_tensor_batch_keys=None, meta_info_keys=None) -> 'DataProto':
        """Copy a subset of the DataProto via `batch_keys` and `meta_info_keys`

        Args:
            batch_keys (list, optional): a list of strings indicating the keys in batch to copy
            meta_info_keys (list, optional): a list of keys indicating the meta info to copy

        Returns:
            DataProto: the DataProto with the copied batch_keys and meta_info_keys
        """
        ...
        for key in batch_keys:
            ...
            tensors[key] = self.batch[key]
        non_tensors = {}
        # non tensor batch
        for key in non_tensor_batch_keys:
            assert key in self.non_tensor_batch.keys()
            non_tensors[key] = self.non_tensor_batch[key]
        meta_info = {}
        for key in meta_info_keys:
            assert key in self.meta_info.keys()
            meta_info[key] = self.meta_info[key]
        ...
        for key, val in non_tensor_batch.items():
            if val[1].shape[0] == 1:
                if key == 'raw_prompt_ids':
                    temp = [list(x) for x in val[0]]
                    temp.append(list(val[1][0]))
                    non_tensor_batch[key] = np.array(temp, dtype=object)
                else:
                    temp = [x for x in val[0]]
                    temp.append(val[1][0])
                    non_tensor_batch[key] = np.array(temp, dtype=object)
            else:
...
from cppo_verl.verl.utils.torch_functional import allgather_dict_tensors
